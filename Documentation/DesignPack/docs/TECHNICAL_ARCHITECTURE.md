...
Use the smallest useful developer map and focused deterministic tests. Do not build every map, system and infrastructure layer before the first real-content loop. Per-feature “implemented” status needs evidence in [current handoff](production/CURRENT_HANDOFF.md); this document alone establishes no runtime result.
