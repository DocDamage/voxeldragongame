"""Reproduce the bounded Chef Aurelio / Carver's Precision live PIE proof."""
...
SCRIPT = ROOT / "tools/unreal/verify_wp23_6_chef_aurelio_carvers_precision.py"
...
try:
    ...
    print(f"WP-23.6 Chef Aurelio / Carver's Precision proof failed: {exc}", file=sys.stderr)
    raise SystemExit(1)
if proof.get("status") != "PASS":
    print(f"WP-23.6 Chef Aurelio / Carver's Precision proof status: {proof.get('status')}", file=sys.stderr)
    raise SystemExit(1)
