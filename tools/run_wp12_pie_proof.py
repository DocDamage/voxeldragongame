...
SCRIPT = (ROOT / "tools/unreal/verify_wp12_region01_proof.py").as_posix()
REPORT = ROOT / "Saved/Diagnostics/WP12_region01_proof.json"
...
print(f"Launching WP-12 Region 01 PIE fact probe with script: {SCRIPT}")
try:
    ...
    print("WP-12 PIE proof timed out after 120 seconds.", file=sys.stderr)
    sys.exit(1)
...
try:
    ...
    print(f"WP-12 PIE report missing or unreadable: {exc}", file=sys.stderr)
    sys.exit(1)
if process.returncode != 0 or result.get("status") != "PASS":
    print(
        f"WP-12 PIE proof failed: process={process.returncode}, "
