"""Reproduce the bounded WP-23.5 Michael Mire gameplay proof."""
...
SCRIPT = ROOT / "tools/unreal/verify_wp23_5_michael_mire_slice.py"
...
try:
    ...
    print(f"WP-23.5 Michael Mire slice failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS_AUTOMATED_REQUIRES_MANUAL_VISUAL_REVIEW":
    print(f"WP-23.5 Michael Mire proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
sys.exit(0)
