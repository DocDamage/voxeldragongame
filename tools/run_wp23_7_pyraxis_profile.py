"""Launch and validate the focused WP-23.7 Pyraxis profile PIE proof."""
...
SCRIPT = (ROOT / "tools/unreal/verify_wp23_7_pyraxis_profile.py").as_posix()
REPORT = ROOT / "Saved/Diagnostics/WP23_7_pyraxis_profile_proof.json"
...
try:
    ...
    print("Pyraxis PIE proof timed out after 180 seconds", file=sys.stderr)
    raise SystemExit(1)
...
try:
    ...
    print(f"Pyraxis PIE report missing or unreadable: {exc}", file=sys.stderr)
    raise SystemExit(1)

if process.returncode != 0 or result.get("status") != "PASS":
    print(
        f"Pyraxis PIE proof failed: process={process.returncode}, "
