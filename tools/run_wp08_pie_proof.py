...
SCRIPT = (ROOT / "tools/unreal/verify_wp08_camp_proof.py").as_posix()
REPORT = ROOT / "Saved/Diagnostics/WP08_camp_proof.json"
...
print(f"Launching WP-08 PIE verification probe with script: {SCRIPT}")
try:
    ...
    print("WP-08 PIE proof timed out after 120 seconds.", file=sys.stderr)
    sys.exit(1)
...
try:
    ...
    print(f"WP-08 PIE report missing or unreadable: {exc}", file=sys.stderr)
    sys.exit(1)
if process.returncode != 0 or result.get("status") != "PASS":
    print(
        f"WP-08 PIE proof failed: process={process.returncode}, "
