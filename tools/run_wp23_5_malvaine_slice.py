"""Reproduce the bounded WP-23.5 Count Malvaine encounter proof."""
...
try:
    ...
    run(EDITOR, ROOT / "tools/unreal/verify_wp23_5_malvaine_slice.py", windowed=True)
    ...
    print(f"WP-23.5 Count Malvaine slice failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS_AUTOMATED_REQUIRES_MANUAL_VISUAL_REVIEW":
    print(f"WP-23.5 Count Malvaine proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
sys.exit(0)
