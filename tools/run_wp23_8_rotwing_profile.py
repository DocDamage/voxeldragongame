"""Launch and validate the focused WP-23.8 Rotwing profile PIE proof."""
...
SCRIPT = (ROOT / "tools/unreal/verify_wp23_8_rotwing_profile.py").as_posix()
REPORT = ROOT / "Saved/Diagnostics/WP23_8_rotwing_profile_proof.json"
...
try:
    ...
    print("Rotwing PIE proof timed out after 180 seconds", file=sys.stderr)
    raise SystemExit(1)
...
try:
    ...
    print(f"Rotwing PIE report missing or unreadable: {exc}", file=sys.stderr)
    raise SystemExit(1)

if process.returncode != 0 or result.get("status") != "PASS":
    print(
        f"Rotwing PIE proof failed: process={process.returncode}, "
