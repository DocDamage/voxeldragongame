"""Reproduce the bounded WP-23.6 Schema 8 Cogspire recovery proof."""
...
REPORT = ROOT / "Saved/Diagnostics/WP23_6_cogspire_save_proof.json"
SCRIPT = ROOT / "tools/unreal/verify_wp23_6_cogspire_save.py"
command = [
...
try:
    ...
    print(f"WP-23.6 Cogspire Schema 8 recovery failed: {exc}", file=sys.stderr)
    raise SystemExit(1)
if proof.get("status") != "PASS":
    print(f"WP-23.6 Cogspire save proof status: {proof.get('status')}", file=sys.stderr)
    raise SystemExit(1)
