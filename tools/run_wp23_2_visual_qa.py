...
SCRIPT = (ROOT / "tools/unreal/capture_wp23_2_visual_qa.py").as_posix()
...
try:
    ...
    print(f"WP-23.2 visual QA report missing or unreadable: {exc}", file=sys.stderr)
    sys.exit(1)
if process.returncode != 0 or result.get("capture_status") != "PASS":
    print(f"WP-23.2 visual QA capture failed: process={process.returncode}, capture={result.get('capture_status')}", file=sys.stderr)
    sys.exit(1)
