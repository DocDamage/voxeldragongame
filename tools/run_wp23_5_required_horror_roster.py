"""Reproduce the complete remaining WP-23.5 required-horror roster proof."""
import json
import subprocess
import sys
...
command = [
    ...
    f"-ExecCmds=py exec(open('{(ROOT / 'tools/unreal/verify_wp23_5_required_horror_roster.py').as_posix()}').read())",
    "-stdout", "-FullStdOutLogOutput"]
try:
    result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=360)
    ...
    print(f"WP-23.5 required-horror roster failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS_AUTOMATED_REQUIRES_MANUAL_VISUAL_REVIEW":
    print(f"WP-23.5 required-horror roster proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
