"""Reproduce the bounded WP-23.5 Gloaming environment/navigation foundation."""
...
try:
    ...
    run(EDITOR, ROOT / "tools/unreal/verify_wp23_5_gloaming_foundation.py", windowed=True)
    ...
    print(f"WP-23.5 Gloaming foundation failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS_AUTOMATED_REQUIRES_MANUAL_VISUAL_REVIEW":
    print(f"WP-23.5 Gloaming proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
sys.exit(0)
