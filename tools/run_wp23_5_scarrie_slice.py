"""Reproduce the bounded WP-23.5 Scarrie gameplay proof."""
import json
import subprocess
import sys
...
command = [
    ...
    f"-ExecCmds=py exec(open('{(ROOT / 'tools/unreal/verify_wp23_5_scarrie_slice.py').as_posix()}').read())",
...
try:
    ...
    print(f"WP-23.5 Scarrie slice failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS_AUTOMATED_REQUIRES_MANUAL_VISUAL_REVIEW":
    print(f"WP-23.5 Scarrie proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
