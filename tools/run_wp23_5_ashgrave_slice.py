"""Reproduce the bounded WP-23.5 Arrival-to-Ashgrave gameplay proof."""
...
try:
    ...
    run(EDITOR, ROOT / "tools/unreal/verify_wp23_5_ashgrave_slice.py", windowed=True)
    ...
    print(f"WP-23.5 Arrival-to-Ashgrave slice failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS":
    print(f"WP-23.5 Arrival-to-Ashgrave proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
sys.exit(0)
