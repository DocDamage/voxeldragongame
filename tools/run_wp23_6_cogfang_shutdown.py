"""Reproduce the bounded WP-23.6 captive Cogfang selective shutdown proof."""
...
REPORT = ROOT / "Saved/Diagnostics/WP23_6_cogfang_shutdown_proof.json"
SCRIPT = ROOT / "tools/unreal/verify_wp23_6_cogfang_shutdown.py"
command = [
...
try:
    ...
    print(f"WP-23.6 Cogfang shutdown slice failed: {exc}", file=sys.stderr)
    raise SystemExit(1)
if proof.get("status") != "PASS":
    print(f"WP-23.6 Cogfang shutdown proof status: {proof.get('status')}", file=sys.stderr)
    raise SystemExit(1)
