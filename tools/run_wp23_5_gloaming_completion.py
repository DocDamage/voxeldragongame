"""Reproduce the WP-23.5 Gloaming regional-completion proof."""
import json
import subprocess
import sys
...
command = [
    ...
    f"-ExecCmds=py exec(open('{(ROOT / 'tools/unreal/verify_wp23_5_gloaming_completion.py').as_posix()}').read())",
...
try:
    ...
    print(f"WP-23.5 Gloaming completion failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS":
    print(f"WP-23.5 Gloaming completion proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
