"""Launch and validate the focused WP-23.6 Cogfang profile PIE proof."""
...
SCRIPT = (ROOT / "tools/unreal/verify_wp23_6_cogfang_profile.py").as_posix()
REPORT = ROOT / "Saved/Diagnostics/WP23_6_cogfang_profile_proof.json"
...
try:
    ...
    print("Cogfang PIE proof timed out after 180 seconds", file=sys.stderr)
    raise SystemExit(1)
...
try:
    ...
    print(f"Cogfang PIE report missing or unreadable: {exc}", file=sys.stderr)
    raise SystemExit(1)

if process.returncode != 0 or result.get("status") != "PASS":
    print(
        f"Cogfang PIE proof failed: process={process.returncode}, "
