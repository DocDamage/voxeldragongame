"""Exercise the bounded Wherewolf required-horror encounter in live PIE."""
import json
import time
import traceback
...
OUT = ROOT / "Saved/Diagnostics/WP23_5_Wherewolf"
...
if not any(isinstance(a, unreal.WyrmCharacter) for a in actors.get_all_level_actors()):
    player = actors.spawn_actor_from_class(unreal.WyrmCharacter, unreal.Vector(3500, 2500, 910), unreal.Rotator())
    player.set_actor_label("DIAG_WP23_5_Wherewolf_Player")
camera_origin = unreal.Vector(3000, 3150, 1050)
camera_target = unreal.Vector(3300, 2900, 1010)
camera = actors.spawn_actor_from_class(
    unreal.CameraActor, camera_origin, unreal.MathLibrary.find_look_at_rotation(camera_origin, camera_target))
camera.set_actor_label("DIAG_GLM_WherewolfEncounter")
OUT.mkdir(parents=True, exist_ok=True)
report = {"kind": "wp23_5_wherewolf_encounter_slice", "engine": unreal.SystemLibrary.get_engine_version(),
...
def prerequisites(region):
    return (region.record_arrival() and region.resolve_ashgrave_extraction_seal() and
            ...
            region.record_pleatherface_resolution())
...
def tick(_delta):
    ...
    try:
        ...
        if wait_ticks:
            ...
            return
        if stage == "capture":
            screenshot = OUT / "01_wherewolf_encounter.png"
        ...
        encounters = world_actors(world, unreal.WyrmWherewolfCharacter)
        cameras = [a for a in world_actors(world, unreal.CameraActor) if a.get_actor_label() == "DIAG_GLM_WherewolfEncounter"]
        ...
        if len(players) != 1 or len(encounters) != 1 or len(cameras) != 1 or len(adapters) != 1:
            return
        if "GLM_ROUTE_PLEATHERFACE" not in anchors or "GLM_ROUTE_WHEREWOLF" not in anchors:
            raise RuntimeError("Missing Pleatherface-to-Wherewolf route anchors")
        player, wolf = players[0], encounters[0]
        ...
        def project(location):
            result = unreal.WyrmTerrainDiagnostics.project_navigation_point(wolf, location, unreal.Vector(240, 240, 500))
        ...
        start_ok, start = project(anchors["GLM_ROUTE_PLEATHERFACE"].get_actor_location())
        end_ok, end = project(anchors["GLM_ROUTE_WHEREWOLF"].get_actor_location())
        if not start_ok or not end_ok:
            ...
            if nav_attempts < 600:
                return
            raise RuntimeError("Wherewolf route anchors did not project")
        path_points = int(unreal.WyrmTerrainDiagnostics.find_complete_navigation_path_point_count(wolf, start, end))
        parts = wolf.get_components_by_class(unreal.StaticMeshComponent)
        meshes = [p.get_editor_property("static_mesh") for p in parts]
        presentation_ok = bool(wolf.has_complete_presentation() and wolf.get_presentation_part_count() == 16 and
            len(parts) == 16 and all(meshes) and all("Werewolf" in m.get_path_name() for m in meshes) and
            all(p.get_material(0) for p in parts) and 204.0 <= wolf.get_presentation_height() <= 206.0)
        region.reset_gloaming_state()
        wolf.reset_encounter()
        rejected = not wolf.begin_encounter(player)
        prereqs = prerequisites(region)
        began = wolf.begin_encounter(player)
        attrs = wolf.get_attributes()
        ...
        while attrs.get_current_health() > attrs.get_current_max_health() * 0.25 and strikes < 12:
            unreal.WyrmMeleeAttackAbility.apply_damage_effect(player.get_ability_system(), wolf.get_ability_system(), 80.0)
            strikes += 1
        submission_health = attrs.get_current_health()
        threshold = 0.0 < submission_health <= attrs.get_current_max_health() * 0.25
        resolved = wolf.resolve_after_calmed_submission(player)
        committed = region.has_fact(unreal.Name("gloaming.wherewolf_resolved")) and region.has_receipt(unreal.Name("gloaming.wherewolf.calmed_submission"))
        duplicate_rejected = not wolf.resolve_after_calmed_submission(player)
        snapshot = unreal.WyrmSaveSubsystem.create_snapshot_object("WP23_5_WherewolfMemory", player, adapters[0], world)
        region.reset_gloaming_state()
        restored = bool(snapshot and unreal.WyrmSaveSubsystem.apply_snapshot_object(snapshot, player, adapters[0], world) and
            region.has_fact(unreal.Name("gloaming.wherewolf_resolved")) and region.has_receipt(unreal.Name("gloaming.wherewolf.calmed_submission")))
        excluded = not region.has_fact(unreal.Name("gloaming.region_complete")) and not region.has_fact(unreal.Name("echo.wherewolf")) and not player.is_echo_unlocked("Wherewolf")
        report["acceptance"] = {
            "pleatherface_anchor_projects_to_navigation": start_ok,
            "wherewolf_anchor_projects_to_navigation": end_ok,
            "pleatherface_to_wherewolf_path_valid_nonpartial": path_points > 0,
            "supplied_16_part_205cm_presentation": presentation_ok,
            "encounter_rejects_before_pleatherface": rejected,
            "gas_calmed_submission_commits_once": bool(prereqs and began and threshold and resolved and committed and duplicate_rejected),
            "gloaming_ledger_roundtrip_restores_receipt": restored,
            "echo_and_regional_completion_absent": excluded}
        report["measurements"] = {"part_count": len(parts), "presentation_height_cm": wolf.get_presentation_height(),
        ...
        wolf.set_actor_location(unreal.Vector(3300, 2900, 910.5), False, True)
        wolf.set_actor_rotation(unreal.Rotator(0, 0, -35), False)
        ...
        REPORT.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        unreal.log_error(f"WP23_5_WHEREWOLF_FAIL {stage}: {exc!r}")
        finished = True
...
unreal.log("WP-23.5 Wherewolf PIE proof started")
