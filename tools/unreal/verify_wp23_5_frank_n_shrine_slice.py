"""Exercise the bounded Frank N. Shrine required-horror encounter in live PIE."""
import json
import time
import traceback
...
REPORT = ROOT / "Saved/Diagnostics/WP23_5_frank_n_shrine_slice_proof.json"
OUT = ROOT / "Saved/Diagnostics/WP23_5_FrankNShrine"
...
if not any(isinstance(a, unreal.WyrmCharacter) for a in actors.get_all_level_actors()):
    player = actors.spawn_actor_from_class(unreal.WyrmCharacter, unreal.Vector(3950, 4550, 904), unreal.Rotator())
    player.set_actor_label("DIAG_WP23_5_FrankNShrine_Player")
camera_origin = unreal.Vector(4500, 5100, 1050)
camera_target = unreal.Vector(4150, 4750, 990)
camera = actors.spawn_actor_from_class(
    unreal.CameraActor, camera_origin, unreal.MathLibrary.find_look_at_rotation(camera_origin, camera_target))
camera.set_actor_label("DIAG_GLM_FrankNShrineEncounter")
OUT.mkdir(parents=True, exist_ok=True)
report = {"kind": "wp23_5_frank_n_shrine_encounter_slice", "engine": unreal.SystemLibrary.get_engine_version(),
...
def prerequisites(region):
    return (region.record_arrival() and region.resolve_ashgrave_extraction_seal() and
            ...
            region.record_chuckles_resolution() and region.record_count_dripula_resolution())
...
def tick(_delta):
    ...
    try:
        ...
        if wait_ticks:
            ...
            return
        if stage == "capture":
            screenshot = OUT / "01_frank_n_shrine_encounter.png"
        ...
        encounters = world_actors(world, unreal.WyrmFrankNShrineCharacter)
        cameras = [a for a in world_actors(world, unreal.CameraActor) if a.get_actor_label() == "DIAG_GLM_FrankNShrineEncounter"]
        ...
        if len(players) != 1 or len(encounters) != 1 or len(cameras) != 1 or len(adapters) != 1:
            return
        if "GLM_ROUTE_COUNT_DRIPULA" not in anchors or "GLM_ROUTE_FRANK_N_SHRINE" not in anchors:
            raise RuntimeError("Missing Count-Dripula-to-Frank-N-Shrine route anchors")
        player, frank = players[0], encounters[0]
        ...
        def project(location):
            result = unreal.WyrmTerrainDiagnostics.project_navigation_point(frank, location, unreal.Vector(240, 240, 500))
        ...
        start_ok, start = project(anchors["GLM_ROUTE_COUNT_DRIPULA"].get_actor_location())
        end_ok, end = project(anchors["GLM_ROUTE_FRANK_N_SHRINE"].get_actor_location())
        if not start_ok or not end_ok:
            ...
            if nav_attempts < 600:
                return
            raise RuntimeError("Frank N. Shrine route anchors did not project")
        path_points = int(unreal.WyrmTerrainDiagnostics.find_complete_navigation_path_point_count(frank, start, end))
        parts = frank.get_components_by_class(unreal.StaticMeshComponent)
        meshes = [p.get_editor_property("static_mesh") for p in parts]
        presentation_ok = bool(frank.has_complete_presentation() and frank.get_presentation_part_count() == 16 and
            len(parts) == 16 and all(meshes) and all("Frankenstein" in m.get_path_name() for m in meshes) and
            all(p.get_material(0) for p in parts) and 201.0 <= frank.get_presentation_height() <= 203.0)
        region.reset_gloaming_state()
        frank.reset_encounter()
        rejected = not frank.begin_encounter(player)
        prereqs = prerequisites(region)
        began = frank.begin_encounter(player)
        attrs = frank.get_attributes()
        ...
        while attrs.get_current_health() > attrs.get_current_max_health() * 0.25 and strikes < 12:
            unreal.WyrmMeleeAttackAbility.apply_damage_effect(player.get_ability_system(), frank.get_ability_system(), 105.0)
            strikes += 1
        submission_health = attrs.get_current_health()
        threshold = 0.0 < submission_health <= attrs.get_current_max_health() * 0.25
        resolved = frank.resolve_after_grounded_submission(player)
        committed = region.has_fact(unreal.Name("gloaming.frank_n_shrine_resolved")) and region.has_receipt(unreal.Name("gloaming.frank_n_shrine.grounded_submission"))
        duplicate_rejected = not frank.resolve_after_grounded_submission(player)
        snapshot = unreal.WyrmSaveSubsystem.create_snapshot_object("WP23_5_FrankNShrineMemory", player, adapters[0], world)
        region.reset_gloaming_state()
        restored = bool(snapshot and unreal.WyrmSaveSubsystem.apply_snapshot_object(snapshot, player, adapters[0], world) and
            region.has_fact(unreal.Name("gloaming.frank_n_shrine_resolved")) and region.has_receipt(unreal.Name("gloaming.frank_n_shrine.grounded_submission")))
        excluded = not region.has_fact(unreal.Name("gloaming.region_complete")) and not region.has_fact(unreal.Name("echo.frank_n_shrine")) and not player.is_echo_unlocked("FrankNShrine")
        report["acceptance"] = {
            "count_dripula_anchor_projects_to_navigation": start_ok,
            "frank_n_shrine_anchor_projects_to_navigation": end_ok,
            "count_dripula_to_frank_n_shrine_path_valid_nonpartial": path_points > 0,
            "supplied_16_part_202cm_presentation": presentation_ok,
            "encounter_rejects_before_count_dripula": rejected,
            "gas_grounded_submission_commits_once": bool(prereqs and began and threshold and resolved and committed and duplicate_rejected),
            "gloaming_ledger_roundtrip_restores_receipt": restored,
            "echo_and_regional_completion_absent": excluded}
        report["measurements"] = {"part_count": len(parts), "presentation_height_cm": frank.get_presentation_height(),
        ...
        frank.set_actor_location(unreal.Vector(4150, 4750, 909), False, True)
        frank.set_actor_rotation(unreal.Rotator(0, 0, 135), False)
        ...
        REPORT.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        unreal.log_error(f"WP23_5_FRANK_N_SHRINE_FAIL {stage}: {exc!r}")
        finished = True
...
unreal.log("WP-23.5 Frank N. Shrine PIE proof started")
