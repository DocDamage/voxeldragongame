"""Exercise the bounded Annie Wails required-horror encounter in live PIE."""
import json
import time
import traceback
...
REPORT = ROOT / "Saved/Diagnostics/WP23_5_annie_wails_slice_proof.json"
OUT = ROOT / "Saved/Diagnostics/WP23_5_AnnieWails"
...
if not any(isinstance(a, unreal.WyrmCharacter) for a in actors.get_all_level_actors()):
    player = actors.spawn_actor_from_class(unreal.WyrmCharacter, unreal.Vector(2900, 2400, 904), unreal.Rotator())
    player.set_actor_label("DIAG_WP23_5_AnnieWails_Player")
camera_origin = unreal.Vector(2925, 2425, 1015)
camera_target = unreal.Vector(2650, 2250, 980)
camera = actors.spawn_actor_from_class(
    unreal.CameraActor, camera_origin, unreal.MathLibrary.find_look_at_rotation(camera_origin, camera_target))
camera.set_actor_label("DIAG_GLM_AnnieWailsEncounter")
OUT.mkdir(parents=True, exist_ok=True)
report = {"kind": "wp23_5_annie_wails_encounter_slice", "engine": unreal.SystemLibrary.get_engine_version(),
...
def prerequisites(region):
    return (region.record_arrival() and region.resolve_ashgrave_extraction_seal() and
            ...
            region.record_pleatherface_resolution() and region.record_wherewolf_resolution())
...
def tick(_delta):
    ...
    try:
        ...
        if wait_ticks:
            ...
            return
        if stage == "capture":
            screenshot = OUT / "01_annie_wails_encounter.png"
        ...
        encounters = world_actors(world, unreal.WyrmAnnieWailsCharacter)
        cameras = [a for a in world_actors(world, unreal.CameraActor) if a.get_actor_label() == "DIAG_GLM_AnnieWailsEncounter"]
        ...
        if len(players) != 1 or len(encounters) != 1 or len(cameras) != 1 or len(adapters) != 1:
            return
        if "GLM_ROUTE_WHEREWOLF" not in anchors or "GLM_ROUTE_ANNIE_WAILS" not in anchors:
            raise RuntimeError("Missing Wherewolf-to-Annie-Wails route anchors")
        player, annie = players[0], encounters[0]
        ...
        def project(location):
            result = unreal.WyrmTerrainDiagnostics.project_navigation_point(annie, location, unreal.Vector(240, 240, 500))
        ...
        start_ok, start = project(anchors["GLM_ROUTE_WHEREWOLF"].get_actor_location())
        end_ok, end = project(anchors["GLM_ROUTE_ANNIE_WAILS"].get_actor_location())
        if not start_ok or not end_ok:
            ...
            if nav_attempts < 600:
                return
            raise RuntimeError("Annie Wails route anchors did not project")
        path_points = int(unreal.WyrmTerrainDiagnostics.find_complete_navigation_path_point_count(annie, start, end))
        parts = annie.get_components_by_class(unreal.StaticMeshComponent)
        meshes = [p.get_editor_property("static_mesh") for p in parts]
        presentation_ok = bool(annie.has_complete_presentation() and annie.get_presentation_part_count() == 17 and
            len(parts) == 17 and all(meshes) and all("AnnieWilkes" in m.get_path_name() for m in meshes) and
            all(p.get_material(0) for p in parts) and any("Sword_AnnieWilkes" in m.get_path_name() for m in meshes) and
            175.0 <= annie.get_presentation_height() <= 177.0)
        region.reset_gloaming_state()
        annie.reset_encounter()
        rejected = not annie.begin_encounter(player)
        prereqs = prerequisites(region)
        began = annie.begin_encounter(player)
        attrs = annie.get_attributes()
        ...
        while attrs.get_current_health() > attrs.get_current_max_health() * 0.25 and strikes < 12:
            unreal.WyrmMeleeAttackAbility.apply_damage_effect(player.get_ability_system(), annie.get_ability_system(), 85.0)
            strikes += 1
        submission_health = attrs.get_current_health()
        threshold = 0.0 < submission_health <= attrs.get_current_max_health() * 0.25
        resolved = annie.resolve_after_disarmed_surrender(player)
        committed = region.has_fact(unreal.Name("gloaming.annie_wails_resolved")) and region.has_receipt(unreal.Name("gloaming.annie_wails.disarmed_surrender"))
        duplicate_rejected = not annie.resolve_after_disarmed_surrender(player)
        snapshot = unreal.WyrmSaveSubsystem.create_snapshot_object("WP23_5_AnnieWailsMemory", player, adapters[0], world)
        region.reset_gloaming_state()
        restored = bool(snapshot and unreal.WyrmSaveSubsystem.apply_snapshot_object(snapshot, player, adapters[0], world) and
            region.has_fact(unreal.Name("gloaming.annie_wails_resolved")) and region.has_receipt(unreal.Name("gloaming.annie_wails.disarmed_surrender")))
        excluded = not region.has_fact(unreal.Name("gloaming.region_complete")) and not region.has_fact(unreal.Name("echo.annie_wails")) and not player.is_echo_unlocked("AnnieWails")
        report["acceptance"] = {
            "wherewolf_anchor_projects_to_navigation": start_ok,
            "annie_wails_anchor_projects_to_navigation": end_ok,
            "wherewolf_to_annie_wails_path_valid_nonpartial": path_points > 0,
            "supplied_17_part_sword_176cm_presentation": presentation_ok,
            "encounter_rejects_before_wherewolf": rejected,
            "gas_disarmed_surrender_commits_once": bool(prereqs and began and threshold and resolved and committed and duplicate_rejected),
            "gloaming_ledger_roundtrip_restores_receipt": restored,
            "echo_and_regional_completion_absent": excluded}
        report["measurements"] = {"part_count": len(parts), "presentation_height_cm": annie.get_presentation_height(),
        ...
        annie.set_actor_location(unreal.Vector(2650, 2250, 896), False, True)
        annie.set_actor_rotation(unreal.Rotator(0, 0, 25), False)
        ...
        REPORT.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        unreal.log_error(f"WP23_5_ANNIE_WAILS_FAIL {stage}: {exc!r}")
        finished = True
...
unreal.log("WP-23.5 Annie Wails PIE proof started")
