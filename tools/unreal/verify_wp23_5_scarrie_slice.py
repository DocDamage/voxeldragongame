"""Exercise the bounded Scarrie required-horror encounter in live PIE."""
import json
import time
import traceback
...
REPORT = ROOT / "Saved/Diagnostics/WP23_5_scarrie_slice_proof.json"
OUT = ROOT / "Saved/Diagnostics/WP23_5_Scarrie"
...
if not any(isinstance(a, unreal.WyrmCharacter) for a in actors.get_all_level_actors()):
    player = actors.spawn_actor_from_class(unreal.WyrmCharacter, unreal.Vector(2350, 2900, 904), unreal.Rotator())
    player.set_actor_label("DIAG_WP23_5_Scarrie_Player")
camera_origin = unreal.Vector(2450, 3350, 1020)
camera_target = unreal.Vector(2100, 3100, 975)
camera = actors.spawn_actor_from_class(
    unreal.CameraActor, camera_origin, unreal.MathLibrary.find_look_at_rotation(camera_origin, camera_target))
camera.set_actor_label("DIAG_GLM_ScarrieEncounter")
OUT.mkdir(parents=True, exist_ok=True)
report = {"kind": "wp23_5_scarrie_encounter_slice", "engine": unreal.SystemLibrary.get_engine_version(),
...
def prerequisites(region):
    return (region.record_arrival() and region.resolve_ashgrave_extraction_seal() and
            ...
            region.record_annie_wails_resolution())
...
def tick(_delta):
    ...
    try:
        ...
        if wait_ticks:
            ...
            return
        if stage == "capture":
            screenshot = OUT / "01_scarrie_encounter.png"
        ...
        encounters = world_actors(world, unreal.WyrmScarrieCharacter)
        cameras = [a for a in world_actors(world, unreal.CameraActor) if a.get_actor_label() == "DIAG_GLM_ScarrieEncounter"]
        ...
        if len(players) != 1 or len(encounters) != 1 or len(cameras) != 1 or len(adapters) != 1:
            return
        if "GLM_ROUTE_ANNIE_WAILS" not in anchors or "GLM_ROUTE_SCARRIE" not in anchors:
            raise RuntimeError("Missing Annie-Wails-to-Scarrie route anchors")
        player, scarrie = players[0], encounters[0]
        ...
        def project(location):
            result = unreal.WyrmTerrainDiagnostics.project_navigation_point(scarrie, location, unreal.Vector(240, 240, 500))
        ...
        start_ok, start = project(anchors["GLM_ROUTE_ANNIE_WAILS"].get_actor_location())
        end_ok, end = project(anchors["GLM_ROUTE_SCARRIE"].get_actor_location())
        if not start_ok or not end_ok:
            ...
            if nav_attempts < 600:
                return
            raise RuntimeError("Scarrie route anchors did not project")
        path_points = int(unreal.WyrmTerrainDiagnostics.find_complete_navigation_path_point_count(scarrie, start, end))
        parts = scarrie.get_components_by_class(unreal.StaticMeshComponent)
        meshes = [p.get_editor_property("static_mesh") for p in parts]
        presentation_ok = bool(scarrie.has_complete_presentation() and scarrie.get_presentation_part_count() == 16 and
            len(parts) == 16 and all(meshes) and all("Carrie" in m.get_path_name() for m in meshes) and
            all(p.get_material(0) for p in parts) and 173.0 <= scarrie.get_presentation_height() <= 175.0)
        region.reset_gloaming_state()
        scarrie.reset_encounter()
        rejected = not scarrie.begin_encounter(player)
        prereqs = prerequisites(region)
        began = scarrie.begin_encounter(player)
        attrs = scarrie.get_attributes()
        ...
        while attrs.get_current_health() > attrs.get_current_max_health() * 0.25 and strikes < 12:
            unreal.WyrmMeleeAttackAbility.apply_damage_effect(player.get_ability_system(), scarrie.get_ability_system(), 90.0)
            strikes += 1
        submission_health = attrs.get_current_health()
        threshold = 0.0 < submission_health <= attrs.get_current_max_health() * 0.25
        resolved = scarrie.resolve_after_living_submission(player)
        committed = region.has_fact(unreal.Name("gloaming.scarrie_resolved")) and region.has_receipt(unreal.Name("gloaming.scarrie.living_submission"))
        duplicate_rejected = not scarrie.resolve_after_living_submission(player)
        snapshot = unreal.WyrmSaveSubsystem.create_snapshot_object("WP23_5_ScarrieMemory", player, adapters[0], world)
        region.reset_gloaming_state()
        restored = bool(snapshot and unreal.WyrmSaveSubsystem.apply_snapshot_object(snapshot, player, adapters[0], world) and
            region.has_fact(unreal.Name("gloaming.scarrie_resolved")) and region.has_receipt(unreal.Name("gloaming.scarrie.living_submission")))
        excluded = not region.has_fact(unreal.Name("gloaming.region_complete")) and not region.has_fact(unreal.Name("echo.scarrie")) and not player.is_echo_unlocked("Scarrie")
        report["acceptance"] = {
            "annie_wails_anchor_projects_to_navigation": start_ok,
            "scarrie_anchor_projects_to_navigation": end_ok,
            "annie_wails_to_scarrie_path_valid_nonpartial": path_points > 0,
            "supplied_16_part_174cm_presentation": presentation_ok,
            "encounter_rejects_before_annie_wails": rejected,
            "gas_living_submission_commits_once": bool(prereqs and began and threshold and resolved and committed and duplicate_rejected),
            "gloaming_ledger_roundtrip_restores_receipt": restored,
            "echo_and_regional_completion_absent": excluded}
        report["measurements"] = {"part_count": len(parts), "presentation_height_cm": scarrie.get_presentation_height(),
        ...
        scarrie.set_actor_location(unreal.Vector(2100, 3100, 895), False, True)
        scarrie.set_actor_rotation(unreal.Rotator(0, 0, -20), False)
        ...
        REPORT.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        unreal.log_error(f"WP23_5_SCARRIE_FAIL {stage}: {exc!r}")
        finished = True
...
unreal.log("WP-23.5 Scarrie PIE proof started")
