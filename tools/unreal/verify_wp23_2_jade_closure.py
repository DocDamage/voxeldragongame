...
def tick(_delta):
    ...
    try:
        ...
        if stage == "persistence":
            ...
            assert player.get_unseen_hand_remaining_cooldown() > 0.0
            assert all(unreal.WyrmSaveSubsystem.is_schema_version_supported(version) for version in (1, 2, 3, 4, 5))
            assert not unreal.WyrmSaveSubsystem.is_schema_version_supported(0)
            assert not unreal.WyrmSaveSubsystem.is_schema_version_supported(6)
            passed("JC-07", {"schema": 5, "regional_records": regional_ids,
