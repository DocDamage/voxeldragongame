"""Exercise the bounded Count Dripula required-horror encounter in live PIE."""
import json
import time
import traceback
...
REPORT = ROOT / "Saved/Diagnostics/WP23_5_count_dripula_slice_proof.json"
OUT = ROOT / "Saved/Diagnostics/WP23_5_CountDripula"
...
if not any(isinstance(a, unreal.WyrmCharacter) for a in actors.get_all_level_actors()):
    player = actors.spawn_actor_from_class(unreal.WyrmCharacter, unreal.Vector(3400, 4000, 904), unreal.Rotator())
    player.set_actor_label("DIAG_WP23_5_CountDripula_Player")
camera_origin = unreal.Vector(3950, 4550, 1030)
camera_target = unreal.Vector(3600, 4200, 980)
camera = actors.spawn_actor_from_class(
    unreal.CameraActor, camera_origin, unreal.MathLibrary.find_look_at_rotation(camera_origin, camera_target))
camera.set_actor_label("DIAG_GLM_CountDripulaEncounter")
OUT.mkdir(parents=True, exist_ok=True)
report = {"kind": "wp23_5_count_dripula_encounter_slice", "engine": unreal.SystemLibrary.get_engine_version(),
...
def prerequisites(region):
    return (region.record_arrival() and region.resolve_ashgrave_extraction_seal() and
            ...
            region.record_chuckles_resolution())
...
def tick(_delta):
    ...
    try:
        ...
        if wait_ticks:
            ...
            return
        if stage == "capture":
            screenshot = OUT / "01_count_dripula_encounter.png"
        ...
        encounters = world_actors(world, unreal.WyrmCountDripulaCharacter)
        cameras = [a for a in world_actors(world, unreal.CameraActor) if a.get_actor_label() == "DIAG_GLM_CountDripulaEncounter"]
        ...
        if len(players) != 1 or len(encounters) != 1 or len(cameras) != 1 or len(adapters) != 1:
            return
        if "GLM_ROUTE_CHUCKLES" not in anchors or "GLM_ROUTE_COUNT_DRIPULA" not in anchors:
            raise RuntimeError("Missing Chuckles-to-Count-Dripula route anchors")
        player, count_dripula = players[0], encounters[0]
        ...
        def project(location):
            result = unreal.WyrmTerrainDiagnostics.project_navigation_point(count_dripula, location, unreal.Vector(240, 240, 500))
        ...
        start_ok, start = project(anchors["GLM_ROUTE_CHUCKLES"].get_actor_location())
        end_ok, end = project(anchors["GLM_ROUTE_COUNT_DRIPULA"].get_actor_location())
        if not start_ok or not end_ok:
            ...
            if nav_attempts < 600:
                return
            raise RuntimeError("Count Dripula route anchors did not project")
        path_points = int(unreal.WyrmTerrainDiagnostics.find_complete_navigation_path_point_count(count_dripula, start, end))
        parts = count_dripula.get_components_by_class(unreal.StaticMeshComponent)
        meshes = [p.get_editor_property("static_mesh") for p in parts]
        presentation_ok = bool(count_dripula.has_complete_presentation() and count_dripula.get_presentation_part_count() == 16 and
            len(parts) == 16 and all(meshes) and all("Dracula" in m.get_path_name() for m in meshes) and
            all(p.get_material(0) for p in parts) and 183.0 <= count_dripula.get_presentation_height() <= 185.0)
        region.reset_gloaming_state()
        count_dripula.reset_encounter()
        rejected = not count_dripula.begin_encounter(player)
        prereqs = prerequisites(region)
        began = count_dripula.begin_encounter(player)
        attrs = count_dripula.get_attributes()
        ...
        while attrs.get_current_health() > attrs.get_current_max_health() * 0.25 and strikes < 12:
            unreal.WyrmMeleeAttackAbility.apply_damage_effect(player.get_ability_system(), count_dripula.get_ability_system(), 100.0)
            strikes += 1
        submission_health = attrs.get_current_health()
        threshold = 0.0 < submission_health <= attrs.get_current_max_health() * 0.25
        resolved = count_dripula.resolve_after_bloodless_surrender(player)
        committed = region.has_fact(unreal.Name("gloaming.count_dripula_resolved")) and region.has_receipt(unreal.Name("gloaming.count_dripula.bloodless_surrender"))
        duplicate_rejected = not count_dripula.resolve_after_bloodless_surrender(player)
        snapshot = unreal.WyrmSaveSubsystem.create_snapshot_object("WP23_5_CountDripulaMemory", player, adapters[0], world)
        region.reset_gloaming_state()
        restored = bool(snapshot and unreal.WyrmSaveSubsystem.apply_snapshot_object(snapshot, player, adapters[0], world) and
            region.has_fact(unreal.Name("gloaming.count_dripula_resolved")) and region.has_receipt(unreal.Name("gloaming.count_dripula.bloodless_surrender")))
        excluded = not region.has_fact(unreal.Name("gloaming.region_complete")) and not region.has_fact(unreal.Name("echo.count_dripula")) and not player.is_echo_unlocked("CountDripula")
        report["acceptance"] = {
            "chuckles_anchor_projects_to_navigation": start_ok,
            "count_dripula_anchor_projects_to_navigation": end_ok,
            "chuckles_to_count_dripula_path_valid_nonpartial": path_points > 0,
            "supplied_16_part_184cm_presentation": presentation_ok,
            "encounter_rejects_before_chuckles": rejected,
            "gas_bloodless_surrender_commits_once": bool(prereqs and began and threshold and resolved and committed and duplicate_rejected),
            "gloaming_ledger_roundtrip_restores_receipt": restored,
            "echo_and_regional_completion_absent": excluded}
        report["measurements"] = {"part_count": len(parts), "presentation_height_cm": count_dripula.get_presentation_height(),
        ...
        count_dripula.set_actor_location(unreal.Vector(3600, 4200, 900), False, True)
        count_dripula.set_actor_rotation(unreal.Rotator(0, 0, 135), False)
        ...
        REPORT.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
        unreal.log_error(f"WP23_5_COUNT_DRIPULA_FAIL {stage}: {exc!r}")
        finished = True
...
unreal.log("WP-23.5 Count Dripula PIE proof started")
