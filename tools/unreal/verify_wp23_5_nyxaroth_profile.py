"""Focused live-PIE proof for the WP-23.5 Nyxaroth rig/profile gate."""
...
REPORT_PATH = ROOT / "Saved/Diagnostics/WP23_5_nyxaroth_profile_proof.json"
ASSET_BASE = "/Game/WYRMFALL/Development/Intake/WP23_5/Nyxaroth/Dark_Dragon/SkeletalMeshes"
MAP_PACKAGE = "/Game/WYRMFALL/Development/Maps/L_DEV_Bootstrap"

report = {
    "kind": "wp23_5_nyxaroth_profile_proof",
    "engine": unreal.SystemLibrary.get_engine_version(),
    "status": "INITIALIZING",
    "tests": {
        "NYX-01.AssetsProfileAndBond": {"status": "NOT_RUN"},
        "NYX-02.HeartfoldAndClearance": {"status": "NOT_RUN"},
        "NYX-03.MountAndFlight": {"status": "NOT_RUN"},
        "NYX-04.GASCombatAndDirectControl": {"status": "NOT_RUN"},
        "NYX-05.SaveIdentity": {"status": "NOT_RUN"},
    },
    "details": {},
    "logs": [],
}
...
# Authored obstruction for the fail-closed TrueForm clearance check.
...
def pie_tick(_delta):
    global finished, tick_count, player, controller, dragon
    try:
        if finished:
        ...
        if stage == "assets_profile":
            required = [
                ...
                f"{ASSET_BASE}/Dark_DragonIdle_01.Dark_DragonIdle_01",
                f"{ASSET_BASE}/Dark_DragonFlying_01.Dark_DragonFlying_01",
                f"{ASSET_BASE}/Dark_DragonAttack_01.Dark_DragonAttack_01",
            ]
            missing = [path for path in required if not unreal.load_asset(path)]
            if missing:
                raise RuntimeError(f"Missing Nyxaroth assets: {missing}")
            ...
            if not dragon:
                raise RuntimeError("Could not spawn Nyxaroth fixture")
            dragon.set_dragon_id("Nyxaroth")
            if not dragon.has_supported_rig_profile():
                raise RuntimeError("Nyxaroth profile is not supported")
            if len(dragon.follower_mesh_components) != 32:
                raise RuntimeError(f"Expected 32 followers, got {len(dragon.follower_mesh_components)}")
            dragon.perform_boss_defeat()
            if dragon.get_dragon_role() != unreal.WyrmDragonRole.DEFEATED_ALIVE:
                raise RuntimeError("Nyxaroth did not enter living-defeat state")
            if not dragon.bond_with_humanoid(player):
                raise RuntimeError("Nyxaroth bond conversion failed")
            if dragon.get_dragon_role() != unreal.WyrmDragonRole.ALLIED_COMPANION:
                raise RuntimeError("Nyxaroth did not become an allied companion")
            companion = capsule_size(dragon)
            if any(abs(value - expected) > 1.0 for value, expected in zip(companion, (28.0, 34.0))):
                raise RuntimeError(f"Unexpected Companion envelope: {companion}")
            finish("NYX-01.AssetsProfileAndBond", {
        ...
        if stage == "heartfold_wait_grow":
            ...
            if dragon.get_dragon_form() != unreal.WyrmDragonForm.TRUE_FORM:
                raise RuntimeError("Nyxaroth did not reach TrueForm")
            true_form = capsule_size(dragon)
            if any(abs(value - expected) > 1.0 for value, expected in zip(true_form, (115.0, 155.0))):
        ...
        if stage == "heartfold_blocked":
            ...
            if bool_result(result):
                raise RuntimeError("Low ceiling did not block Nyxaroth growth")
            reason = reason_result(result)
            dragon.set_actor_location(unreal.Vector(250.0, 0.0, 120.0), False, False)
            finish("NYX-02.HeartfoldAndClearance", {
                "growth": True,
                "shrink": True,
                "blocked_growth": True,
        ...
        if stage == "mount":
            ...
            if not bool_result(dragon.can_mount(player)) or not dragon.mount_humanoid(player):
                raise RuntimeError("Nyxaroth mount failed")
            if not bool_result(dragon.can_take_off()) or not dragon.take_off():
                raise RuntimeError("Nyxaroth takeoff failed")
            if not dragon.is_in_flight():
                raise RuntimeError("Nyxaroth did not enter flight")
            advance("land")
            return

        if stage == "land":
            if not dragon.land() or dragon.is_in_flight():
                raise RuntimeError("Nyxaroth landing failed")
            if not bool_result(dragon.dismount_humanoid()):
                raise RuntimeError("Nyxaroth dismount failed")
            finish("NYX-03.MountAndFlight", {
                "mount": True, "takeoff": True, "landing": True, "dismount": True,
                "mount_offset": [0.0, 0.0, 150.0], "flight_speed": 1650.0,
            })
        ...
        if stage == "control_combat":
            if not dragon.start_direct_control(controller, player):
                raise RuntimeError("Nyxaroth direct control failed")
            ...
            finish("NYX-04.GASCombatAndDirectControl", {
        ...
        if stage == "save":
            ...
            if str(record.dragon_id) != "Nyxaroth":
            ...
            if str(restored.dragon_id) != "Nyxaroth" or not restored.has_supported_rig_profile():
                raise RuntimeError("Nyxaroth identity/profile did not restore")
            if len(restored.follower_mesh_components) != 32:
                raise RuntimeError("Nyxaroth follower assembly did not restore")
            finish("NYX-05.SaveIdentity", {
        ...
        log(f"PIE proof failed: {exc}\n{traceback.format_exc()}")
        if stage in report["tests"]:
            report["tests"][stage]["status"] = "FAIL"
        report["status"] = "FAIL"
        write_report()
        finished = True
        level_editor.editor_request_end_play()
...
log("Starting focused Nyxaroth profile PIE proof")
