"""Reproduce the bounded WP-23.5 Pleatherface gameplay proof."""
import json
import subprocess
import sys
...
command = [
    ...
    f"-ExecCmds=py exec(open('{(ROOT / 'tools/unreal/verify_wp23_5_pleatherface_slice.py').as_posix()}').read())",
...
try:
    ...
    print(f"WP-23.5 Pleatherface slice failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS_AUTOMATED_REQUIRES_MANUAL_VISUAL_REVIEW":
    print(f"WP-23.5 Pleatherface proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
