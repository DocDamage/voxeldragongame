"""Reproduce the bounded WP-23.5 Second Turn Echo proof."""
...
SCRIPT = ROOT / "tools/unreal/verify_wp23_5_second_turn_slice.py"
...
try:
    ...
    print(f"WP-23.5 Second Turn slice failed: {exc}", file=sys.stderr)
    sys.exit(1)
if proof.get("status") != "PASS":
    print(f"WP-23.5 Second Turn proof status: {proof.get('status')}", file=sys.stderr)
    sys.exit(1)
sys.exit(0)
