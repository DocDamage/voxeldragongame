"""Audit supplied Frosthold, Frostmane, and ruler source candidates."""
...
REPORT = ROOT / "Saved/Diagnostics/WP23_4_frosthold_source_fit.json"

MODEL_SUFFIXES = {".fbx", ".glb", ".gltf", ".obj", ".vox", ".dae", ".blend"}
SEARCH_TERMS = {
    "winter_environment": (
        "snow", "snowy", "winter", "ice", "icy", "frost", "frozen",
        "arctic", "glacier", "igloo",
    ),
    "fortification": ("castle", "keep", "fort", "fortress", "tower", "gate", "wall"),
    "ruler": (
        "king", "queen", "monarch", "lord", "chieftain", "jarl", "viking",
        "commander", "captain", "champion",
    ),
}
...
def categories(name: str) -> dict[str, list[str]]:
    ...
    for category, terms in SEARCH_TERMS.items():
        found = []
        for term in terms:
            if term in {"ice", "icy", "king", "lord", "jarl"}:
...
def main() -> None:
    ...
    winter_hits = [match for match in all_matches if "winter_environment" in match["categories"]]
    ruler_hits = [match for match in all_matches if "ruler" in match["categories"]]
    report = {
        "kind": "wp23_4_frosthold_source_fit",
        "status": "PASS_DRAGON_SOURCE_WITH_REGION_AND_RULER_REVIEW_GATES",
        "scope": "Source inventory only; no Unreal import, rig, rendered, map, gameplay, or persistence claim",
        "frostmane": dragon_row(),
        ...
        "match_counts": counts,
        "winter_environment_candidates": winter_hits,
        "ruler_candidates": ruler_hits,
        "assessment": {
            "frostmane": "READY_FOR_UNREAL_INTAKE",
            "winter_environment": "CANDIDATES_REQUIRE_COMPLETENESS_AND_RENDERED_REVIEW",
            "king_alaric_frostborne": "CANDIDATES_REQUIRE_DISTINCT_RENDERED_REVIEW",
            "moonbound_continuity": "PRESERVE_EXISTING_WP21_IDENTITY_AND_POWER",
        },
        "prohibited_inference": (
            "Keyword hits are discovery candidates only. Snowy trees, generic keeps, and a generic King do not "
            "alone establish a complete storybook Frosthold region or a distinct King Alaric presentation."
        ),
        "next_bounded_task": (
            "Import and measure White Dragon plus the strongest winter-environment and ruler candidates; "
            "do not start Frosthold regional gameplay."
        ),
    }
    REPORT.parent.mkdir(parents=True, exist_ok=True)
    REPORT.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(f"WP-23.4 Frosthold source fit: {report['status']}")
    print(f"Frostmane source: {report['frostmane']['mesh_count']} meshes, {report['frostmane']['animation_count']} animations")
